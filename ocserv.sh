...
Download_ocserv(){
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/cookjohn/one/main/ocserv.conf"
...
Set_iptables(){
	...
	echo -e "net.ipv4.tcp_mtu_probing = 1" >> /etc/sysctl.conf
